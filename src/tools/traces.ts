...
export function registerTraceTools(server: McpServer, client: KibanaClient) {
  server.tool(
    ...
    "특정 트랜잭션의 샘플 트레이스(traceId/transactionId) 목록을 조회합니다. sampleRangeFrom/To(마이크로초)로 느린 트랜잭션만 골라낼 수 있고, 여기서 얻은 traceId를 get_trace에 넘겨 span 단위 병목을 분석합니다",
    {
  ...
  server.tool(
    "get_trace",
    "trace ID로 트랜잭션 전체의 span 워터폴을 조회합니다. 응답은 parent-child 트리(자식은 duration 내림차순)와 self time 기준 상위 5개 병목 목록(topBottlenecks)으로 가공되어 반환됩니다. traceId는 get_transaction_samples로 얻습니다",
    {
